...
lowest terms.  The nonnegative-parameter conjecture originates in the
record for \href{https://oeis.org/A357513}{OEIS A357513}.  We prove the
corresponding classification for every integer $m$, determining exactly
when $p^4$ divides $u_m(p-1)$.
For every prime $p$,
$$
 p^4\mid u_m(p-1)
...
Let $m$ be an integer and let $p$ be prime.  Then
$$
 p^4\mid u_m(p-1)
 \quad\Longleftrightarrow\quad
 p\notin\mathcal E_m.
$$
If $p\in\mathcal E_m$, then
$$
...
\begin{remark}
The exceptional set need not be finite: when $m=-2$, it consists of
all primes except $3$.  For $m\neq-2$, it is finite.
\end{remark}

\section{Common denominators}

Fix $m\in\Z$, and write
$$
 d=2m+1.
$$
For an integer $t$, put
$$
 t_+=\max(t,0),\qquad t_-=\max(-t,0).
$$
Thus $t_+$ and $t_-$ are nonnegative integers and, for $k\neq0$,
$$
 \frac{1}{k^t}=\frac{k^{t_-}}{k^{t_+}}.
$$
For a prime $p$, let
$$
...
$$
 ...
 k^{d_-}\frac{D_m}{k^{d_+}}.
$$
Since $k\mid(p-1)!$ for $1\leq k\leq p-1$, every
$D_m/k^{d_+}$ is an integer.  Thus $N_m\in\Z$,
$S_{m,p}=N_m/D_m$, and $p\nmid D_m$.
...
\end{equation}
Integer powers of elements of $\Zloc$ are understood whenever the base is
a unit.  After squaring \eqref{eq:binom-product} and multiplying by
$k^{-d}$, the $k$th summand of $S_{m,p}$ is
$$
...
The exponent $r$ is odd.  The first-order identity
$$
 (1-x)^{-r}\equiv1+rx\pmod{x^2}
$$
holds for every integer $r$.  Since $k\mapsto p-k$ permutes
$1,\ldots,p-1$,
\begin{align*}
 \frac1{(p-k)^r}
...
$$
For the integer exponent $e$, the standard power-sum evaluation in the
cyclic group
$\Fp^\times$ is
